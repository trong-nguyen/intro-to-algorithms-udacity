...
def compute_duration(start, end):
     fm = '%H:%M'
     return (datetime.datetime.strptime(end, fm)
          - datetime.datetime.strptime(start, fm)).seconds

def hhmm(s):
     return map(int, s.split(':'))

def get_earlier_hhmm(h1, h2):
     return h1 if hhmm(h1) <= hhmm(h2) else h2

def get_later_hhmm(h1, h2):
     return h1 if hhmm(h1) >= hhmm(h2) else h2

def can_take_flight(arrival, depature):
     return hhmm(arrival) <= hhmm(depature)
...
def preprocess(flights):
     return {
          f[0]:{
               'origin': f[1],
               'destination': f[2],
               'duration': compute_duration(f[3], f[4]),
               'cost': f[5],
               'departure': f[3],
               'arrival': f[4],
          } for f in flights
...
def encode_node_name(*ss):
     return '|'.join(map(str, ss))

def decode_node_name(name):
     return name.split('|')

def make_graph(flights):
     # first sweep, connections

     origins = {}

     for fn, fd in flights.iteritems():
          og = fd['origin']
          origins[og] = origins.get(og, []) + [fn]

     graph = {}
     a_graph = {}
     for origin, fns in origins.iteritems():
          a_graph[origin] = {}
          for flight in fns:
               node_name = flight # encode_node_name(origin, flight)
               a_graph[node_name] = flight

     for name, flight in a_graph.items():
          if name in origins:
               continue # ignore the formal terminal nodes
          fd = flights[flight]
          weight = {
               'cost': fd['cost'],
          ...
          if destination in origins:
               for connect_flight in origins[destination]:
                    cfd = flights[connect_flight]
                    if can_take_flight(fd['arrival'], cfd['departure']):
                         dname = connect_flight # encode_node_name(destination, connect_flight)
                         graph[name][dname] = {
                              'cost': cfd['cost'],
                              'duration': cfd['duration']
                         }
          else: #probably the terminal
               graph[name][destination] = weight
...
def accumulate(d, d0):
     ad = {}
     for k, v in d.items():
          if k in ['cost', 'duration']:
               ad[k] = d[k] + d0[k]

     return ad

def choose(w1, w2):
     # first by cost, then by duration
     if w1['cost'] == w2['cost']:
          return w2 if w2['duration'] < w1['duration'] else w1
     return w2 if w2['cost'] < w1['cost'] else w1

def select_among(ws):
     k = min((w['cost'], w['duration'], k) for k, w in ws.iteritems())[-1]
     return k

def retrieve_shortest_path(origin, destination, final_dist):
     transits = []
     transit = destination
     while transit != origin and transit != 'root':
          transits.insert(0, transit)
          transit = final_dist[transit]['from']

     return transits


def dijkstra(graph, origin, destination_flights, flights, origins):
     # first distances
     dist = {}
     ...
     final_dist = {}
     while len(final_dist) < len(graph) and dist:
          v = select_among(dist)
          final_dist[v] = dist[v]
          # if True: #v == 803: # type(v) != int or True:
          #      print 'Reaching [{:18}] from [{:15}], total costs [{:3}], took [{:6}]'.format(
          #           v,
          #           final_dist[v]['from'],
          #           # final_dist[v]['arrival'],
          #           final_dist[v]['cost'],
          #           final_dist[v]['duration'],
          #           )
          if v in destination_flights:
               return final_dist
          del dist[v]
          transit = {'from': v}
          for u in graph[v]:
               ...
               weight = accumulate(graph[v][u], final_dist[v])
               if u not in dist:
                    weight.update(transit)
                    dist[u] = weight
               else:
                    weight = choose(dist[u], weight)
                    if weight['cost'] != dist[u]['cost'] or weight['duration'] != dist[u]['duration']:
                         weight.update(transit)
                    dist[u] = weight
     return final_dist
...
def find_best_flights(flights, origin, destination):
     # preprocess flights into [flight_number, origin, destination, duration, cost]
     processed_flights = preprocess(flights)

     # make graph
     graph, origins = make_graph(processed_flights)

     destination_flights = {
          flight for flight, fd in processed_flights.iteritems() if fd['destination'] == destination
     }

     # dijstraks search for min
     final_dist = dijkstra(graph, origin, destination_flights, processed_flights, origins)
     # print json.dumps(graph[803], indent=2)


     transits = None
     for flight in destination_flights:
          if flight in final_dist:
               print 'Found flight {} that connects to {}'.format(flight, destination)
               transits = retrieve_shortest_path(origin, flight, final_dist)
               break



     print 'Transiting via', transits

     return transits


def traverse_flights(flights, path):
     flights = {
          f[0]: f[1:] for f in flights
     }
     cost = 0
     duration = 0
     for flight in path:
          w = flights[flight]
          cost += w[4]
          duration += compute_duration(w[2], w[3])
          print 'Flying on', flight, '{} to {}:'.format(w[0], w[1]), 'costs {}'.format(cost), 'timed', w[2], w[3], 'takes {}'.format(duration)
...
def test_units():
     assert can_take_flight('11:22', '12:30') == True
     assert can_take_flight('09:30', '08:15') == False

     assert get_later_hhmm('09:30', '19:30') == '19:30'
     assert get_earlier_hhmm('01:15', '00:49') == '00:49'

     d0 = {'cost': 11, 'duration': 390}
     d = {'cost': 100, 'duration': 1100}
     assert accumulate(d, d0) == {'cost': 111, 'duration': 1490}

     a = {'cost': 15, 'duration': 1500}
     b = {'cost': 15, 'duration': 150}
     c = {'cost': 10, 'duration': 150}
     ...
     assert select_among(sample) == 'c', 'Sample {} = {} not equal to {}'.format(sample, select_among(sample), c)


def test():
    flights = find_best_flights(all_flights, 'Mt Magnet', 'Fitzroy Crossing')
    expected = [314, 803, 348, 530, 112]
    traverse_flights(all_flights, expected)
